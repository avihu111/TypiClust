...
from pycls.al.plot_settings import *
...
class DCoM:
    ...
    def __init__(self, cfg, lSet, uSet, budgetSize, lSet_deltas, max_delta):
        ...
        self.all_features = ds_utils.load_features(self.ds_name, self.seed, cfg=cfg)
        self.lSet = lSet
        self.uSet = uSet
    ...
    def select_samples(self, model, train_data, data_obj):
        ...
        """
        global plot_episodes_coverage_y
        ...
        DCoM.calculate_coverage(fully_graph, self.lSet, self.lSet_deltas_dict, len(self.relevant_indices))
        del fully_graph

        competence_score = get_competence_score(plot_episodes_coverage_y[-1])
    ...
    def calculate_margin(self, model, train_data, data_obj):
        ...
        images_loader = data_obj.getLabelLoader(indexes=all_images_idx,
                                                batch_size=self.cfg.TRAIN.BATCH_SIZE, data=train_data)
        clf = model.cuda()
        ranks = []
        n_loader = len(images_loader)
        print("len(images_loader): {}".format(n_loader))
    ...
    def calculate_coverage(fully_df, lSet, lSet_deltas_dict, total_data_len):
        """
        add to plot_episodes_coverage_y the current probability coverage.
        """
        global plot_episodes_coverage_y
        ...
        coverage = len(covered_samples) / total_data_len
        plot_episodes_coverage_y.append(coverage)  # each time before the selection
